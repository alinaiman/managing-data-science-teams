# Between Days: Your Week (March 16-23)

A checklist for the week between Day 1 and Day 2.

## Required Reading for Day 2

Complete these before March 23:

- [ ] Camille Fournier — *The Manager's Path*, Ch. 6 ("Managing a Team") — managing up, performance (~30 pp)
- [ ] Andy Grove — *High Output Management*, Ch. 11 ("Performance Reviews") — foundation for calibration (~30 pp)
- [ ] Will Larson — *An Elegant Puzzle*, Part 2 ("Organizations") & §5.3 ("Presenting to Senior Leadership") — org design at scale and executive communication (~80 pp)

## Portfolio Work

Complete these artifacts started in class:

- [ ] **Work Sample Exercise** — Design a take-home or live exercise for your hiring role (Block B template)
- [ ] **Interview Loop** — Map out your full interview process, debrief protocol, and decision framework (Block B template)
- [ ] **Polish your Roadmap** — Refine RICE scores, fill in the Now/Next/Later visualization, add dependencies
- [ ] **Start your Executive Narrative** — Expand beyond the Context and What's Next sections drafted in class
- [ ] **Review your Team Charter and Stakeholder Map** — These feed into your final Manager OS document

## Day 2 Preview

- **Block D (11:00-12:40):** Growth & Performance — career ladders, SBI feedback practice, mock calibration, PGP drafting
- **Block E (13:30-15:10):** Portfolio Workshop — revise your weakest artifact, structured peer review, QBR prep
- **Block F (15:30-17:10):** QBR Simulation — present your roadmap to a mock exec panel

## Reminders

- Check Moodle for your **QBR group assignment** (posted the night before Day 2)
- Bring your laptop with all templates accessible
- The QBR simulation brief will be posted on Moodle after Day 1

---

*Estimated time: 6-8 hours total (reading + artifact work)*
