...
|---|---|---|---|
| 1 | SOC 2 Type II certification | Must | Non-negotiable for handling customer data; required by InfoSec |
| 2 | Native integration with our warehouse (e.g., Snowflake, BigQuery, Redshift) | Must | Must query data in place; no CSV uploads |
| 3 | SSO / SAML support | Must | Required by IT security policy for all SaaS tools |
| 4 | Horizontal scalability (10x current data volume) | Should | Must handle growth without re-platforming within 18 months |
| 5 | Self-serve access for business users (no SQL required) | Should | Reduces ad-hoc request load on the analytics team |
| 6 | API for programmatic access and embedding | Should | Enables integration with internal tools and automated workflows |
